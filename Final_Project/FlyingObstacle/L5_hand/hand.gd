...
func _ready():
  ...
  $handShadow.set_scale( ScaleUp if ScaleUpCurrent else ScaleSide )
...
func _process(delta):
  ...
  $handShadow.position.y -= TravelVector.y * delta
  ...
  if ScaleUpCurrent :
    ...
    $handShadow.set_scale( lerp($handShadow.get_scale(), ScaleSide, ScaleLerp) );
  else :
    $Anchor.set_scale( lerp($Anchor.get_scale(), ScaleUp, ScaleLerp) );
    $handShadow.set_scale( lerp($handShadow.get_scale(), ScaleUp, ScaleLerp) );
