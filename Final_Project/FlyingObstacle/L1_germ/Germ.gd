...
func _ready():
  ...
  $GermShadow.set_scale( ScaleUp if ScaleUpCurrent else ScaleSide )
...
func _process(delta):
  ...
  $GermShadow.position.y -= TravelVector.y * delta
  ...
  if ScaleUpCurrent :
    ...
    $GermShadow.set_scale( lerp($GermShadow.get_scale(), ScaleSide, ScaleLerp) );
  else :
    $Anchor.set_scale( lerp($Anchor.get_scale(), ScaleUp, ScaleLerp) );
    $GermShadow.set_scale( lerp($GermShadow.get_scale(), ScaleUp, ScaleLerp) );
